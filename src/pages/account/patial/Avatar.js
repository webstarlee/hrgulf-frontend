...
export default (props) => {
  ...
  const payload = () => (
    <div className="mt-3">
      <h4 className="h4-responsive text-left grey-text">{t("ACCOUNT.AVATAR.AVATAR")}</h4>
      <form onSubmit={handleSubmit}>
        ...
        <div className="my-avatar-sliders-container mx-auto mt-md-3 text-left">
          <div>
            <label htmlFor="scale">{t("ACCOUNT.AVATAR.SCALE")}</label>
            <input type="range" className="custom-range" min={0.9} max={2} step={0.01} value={scale} onChange={e => setScale(e.target.value)} />
          </div>
          <div>
            <label htmlFor="yPosition">{t("ACCOUNT.AVATAR.BORDER_RADIUS")}</label>
            <input type="range" className="custom-range" min={0} max={50} step={1} value={borderRadius} onChange={e => setBorderRadius(e.target.value)} />
          </div>
          {/*<div>*/}
          {/*  <label htmlFor="xPosition">{t("ACCOUNT.AVATAR.X_POSITION")}</label>*/}
          {/*  <input type="range" className="custom-range" min={0} max={1} step={0.01} value={position.x} onChange={e => setPosition({...position, x: e.target.value})} />*/}
          {/*</div>*/}
          {/*<div>*/}
          {/*  <label htmlFor="yPosition">{t("ACCOUNT.AVATAR.Y_POSITION")}</label>*/}
          {/*  <input type="range" className="custom-range" min={0} max={1} step={0.01} value={position.y} onChange={e => setPosition({...position, y: e.target.value})} />*/}
          {/*</div>*/}
        </div>
